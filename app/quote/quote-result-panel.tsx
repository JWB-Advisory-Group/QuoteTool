    ...
    <div className="rounded-lg border border-[#d8d4c7] bg-white p-5 shadow-sm sm:p-6">
      ...
      {needsPhotos ? (
        <div className="mt-5 rounded-md border border-[#f1d18a] bg-[#fff8e5] px-3 py-3 text-sm font-medium text-[#7a5400]">
          This is an estimated quote. Photos are optional, but 2-4 clear shots
          usually let Dante turn it into an actual quote without a site visit.
          Skip them if you prefer a quick follow-up first.
        </div>
      ) : null}
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        <div className="rounded-md border border-[#e4e0d5] bg-[#fbfaf7] p-4">
          ...
          <div className="mt-3 rounded-md bg-white px-3 py-2 text-xs font-semibold text-[#62685f]">
            {`Confidence: ${result.estimate.estimateConfidence}`}
          </div>
        </div>
        <div className="rounded-md border border-[#e4e0d5] bg-[#fbfaf7] p-4">
      ...
      {result.estimate.pricingNotes.length > 0 ? (
        <div className="mt-5 rounded-md border border-[#e4e0d5] bg-[#fbfaf7] p-4">
          <div className="text-sm font-semibold">How to read this range</div>
          <div className="mt-3 space-y-2">
            {result.estimate.pricingNotes.slice(0, 4).map((note) => (
      ...
      <div className={`mt-6 grid gap-3 ${needsPhotos ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
        {needsPhotos ? (
          <Link
            className="inline-flex min-h-12 items-center justify-center rounded-md bg-[#d8f269] px-4 py-3 text-center text-sm font-semibold leading-5 text-[#1d211c] transition hover:bg-[#c8e65a]"
            href={`/quote/${result.quoteId}#photos`}
          >
